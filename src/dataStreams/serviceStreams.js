...
function getDescribedServices(serviceArns, clusterNameOrArn) {
  const params = {
    ...
    cluster: clusterNameOrArn || 'default',
  };
  return awsRequest.create((awsConfig) => {
    const ecs = new AWS.ECS(awsConfig);
...
export const serviceArnStream$ = Observable.timer(
  0,
  config.SERVICE_ARN_REFRESH_INTERVAL * 1000
) // timer in seconds
  .combineLatest(clusterArnStream$)
  .switchMap(([, clusterArnResponse]) => {
    // foreach cluster arn
    ...
    const serviceArnsRequestObservables$ = clusterArnResponse.clusterArns.map(
      (clusterArn) =>
        Observable.forkJoin(getServiceArns(clusterArn), (resp) => ({
          clusterArn,
          serviceArns: resp,
        }))
    );
...
export const servicesStream$ = Observable.timer(
  0,
  config.SERVICE_REFRESH_INTERVAL * 1000
) // timer in seconds
  ...
  .flatMap(([, clusters]) => {
    ...
    const requestPromises = clusters.map((cluster) => {
      if (cluster.serviceArns.length > 10) {
        ...
        const describeRequestPromises = serviceArnsInChunks.map((serviceArns) =>
          getDescribedServices(serviceArns, cluster.clusterArn)
        );
...
export function aggregatedServiceDeploymentStream$(deploymentCount) {
  ...
  const obs$ = servicesStream$.map((services) => {
    return services
...
export const aggregatedEventStream$ = servicesStream$.map((services) => {
  return services
