...
class State {
  constructor(logCollection, logStreamName, nextToken) {
    if (!logCollection || logCollection.length === 0 || !logStreamName) {
      throw Error(
        'Cannot create a new state with logCollection or logStreamName falsy'
      );
    }
...
class LogViewer extends Component {
  constructor(props) {
    ...
    // bind functions
    this.mapLogCollectionToListBodyComponents =
      this.mapLogCollectionToListBodyComponents.bind(this);
    this.loadMoreButtonClickHandler =
      this.loadMoreButtonClickHandler.bind(this);
  }

  mapLogCollectionToListBodyComponents(events, i) {
    // todo - we should support the case where there is no events
    return <ListBody data={events} key={`listbody-${i}`} />;
  }
  ...
  loadMoreButtonClickHandler() {
    return getLogEvents(this.state.logStreamName).then((nextResult) => {
  ...
  renderEvents() {
    const listBodies = this.state.logCollection.map(
      this.mapLogCollectionToListBodyComponents
    );
    return (
...
class ListBody extends Component {
  mapLogToListItem(log, i) {
    return (
      <li className="log-event-item" key={log.timestamp + i}>
        <em className="timestamp">{moment(log.timestamp).format()}</em> ::{' '}
        {log.message}
      </li>
    );
  }
  ...
  render() {
    ...
    return <ul className="log-event-list-chunk">{listItems}</ul>;
  }
}
...
class LoadMoreButton extends Component {
  ...
  handleClick(e) {
    ...
    this.loading(true);
    this.props.onClick_promise().then(setLoadingFalse).catch(setLoadingFalse);
  }
  ...
  renderLoadingSpinner() {
    ...
    return <i className="spinner left" style={style}></i>;
  }
  ...
  render() {
    return (
      <button
        ...
        className="load-more-button waves-effect waves-light btn"
      >
